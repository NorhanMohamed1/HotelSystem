...
function GetAllAssetType() {
    ...
    if (culture == "ar-SA") {
        var ss = "<option value=0>اختار نوع الاصول </option>";
    }
    else { var ss = "<option value=0> select Asset Type </option>"; } 
...
function GetAllContactType() {
    if (culture == "ar-SA") {
        var ss = "<option value=0>  جهات الإتصال </option>";
    }
    else { var ss = "<option value=0>select contact Type</option>"; }
...
function GetAllCustomerInHotel() {
    ...
    $.get("/Settings/GetAllCustomerInHotel", function (data) {
        $.each(data, function (i, j) {
            ...
            ss = ss + "<td class='align-middle text-center text-sm'>" + j.isCompony + "</td>";
            ...
            ss = ss + "<td class='float-end me-4' ><button   id='DeleteForm'  data-bs-toggle='modal' data-bs-target='#DeleteModal'  class='btn btn-link text-danger text-gradient px-3 mb-0' onclick='DeleteFormCustomer(" + j.id + ")'><i class='fa-regular fa-trash-can me-2' aria-hidden='true'></i></button></td>";
    ...
    $.get("/Settings/GetAllRoomInHotel", function (data) {
        ...
        $.each(data, function (i, j) {
            ...
            if ((j.status == 'مشغوله' && j.contractDuration != 0 && culture == "ar-SA") || (j.status == 'Reserved' && j.contractDuration != 0 && culture == "ar-SA")) {
                ...
                //flatDiv += '<button type="button"     class="btn btn-outline-primary w-25 px-0 py-2 m-1 rounded-2" style="font-size:1vw;" > سند صرف </button>';
                ...
                //flatDiv += '<button type="button"     class="btn btn-outline-primary w-25 px-0 py-2 m-1 rounded-2"  style="font-size:0.9vw;"> Payment Voucher </button>';
...
function changeRoom() {
    ...
    if (id > 0) {
        $.get("/Settings/GetRoom/" + roomid, function (data) {
            ...
            $("#unitPrice").val(data.normalPrice);
            //alert(data.normalPrice);
        });
    }
}
